...
const UserLikesMain = function ({user, count, dateStart}: Props) {
  const getArtworks = async (supabase: SupabaseClient, rangeFrom: number, step: number): Promise<Artwork[]> => {
    ...
    const {data: likedPictures} = await supabase.from('artworks_likes').select('artwork_id').eq('user_id', user?.id || '').order('created_at', {ascending: false}).range(rangeFrom, rangeFrom + step).lte('created_at', dateStart);

    if (likedPictures) {
      for (const likedArt of likedPictures) {
        const {data: artworks} = await supabase.from('artworks').select().eq('id', likedArt.artwork_id);
