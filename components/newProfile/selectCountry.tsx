  ...
  return (
    <div className="relative w-full">
      ...
      <button
        ...
        className={cn(
          'border-t-main border-2 flex w-full  appearance-none items-start pt-1 justify-center leading-none outline-none bg-t-main-2 h-[35px] rounded-md px-2 text-base text-t-hover-1 hover:bg-t-main-2/70',
          {
            'text-t-main': !country,
      ...
      {open && (
        ...
        >
          ...
          <div className="flex flex-col overflow-y-scroll h-[250px]">
            {COUNTRIES.filter((v) =>
              v.name.toLowerCase().includes(searchCountry.toLowerCase())
            ).map((v) => {
              return (
                <button
