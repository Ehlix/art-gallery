...
export const SearchBar = () => {
  ...
  return (
    <div
      ref={menuRef}
      className="group relative flex h-[35px] shrink grow items-center rounded-full bg-grad-1"
    >
      ...
      <div
        className={cn(
          "h-50 absolute left-0 top-full z-10 mt-2 flex w-full flex-col items-start justify-center rounded-md bg-t-main-3 p-3 text-lg text-t-main shadow-lg shadow-t-shadow-1",
          {
            hidden: !open,
          },
        )}
      >
        {result.map((v) => (
          ...
          >
            ...
            <span className="w-full">{`${v.name} (${v.site})`}</span>
          </Link>
        ))}
      </div>
    </div>
