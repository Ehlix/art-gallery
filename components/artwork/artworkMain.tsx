...
export const ArtworkMain = ({
  ...
  return (
    <div className="flex h-full w-full gap-5 md:flex-col">
      <div className="flex h-full w-full flex-col gap-5 overflow-hidden rounded-md md:order-2">
        {artwork.files?.map((v) => (
          <div
            key={v}
      ...
      <div className="relative flex h-fit flex-col gap-5 w-[600px] md:w-full">
        <div className="flex h-fit flex-col gap-5 rounded-md p-5 bg-t-main/20">
          <div className="absolute top-0 right-0 p-2 transition duration-300 hover:text-t-hover-1">
          ...
          <div className="flex flex-col gap-2">
            <h2 className="text-2xl text-t-hover-1">{artwork.title}</h2>
            <p>{artwork.description}</p>
          </div>
          <p className="text-sm italic">{createdAt}</p>
        </div>

        <ArtworkComments artwork_id={artwork.id} currentUser={currentUser} />
